...
async def _placed_by_name(place: MapsPlace, near: tuple[float, float] | None) -> MapsPlace:
    ...
    if place.lat is not None or not place.name:
        return place
    try:
        hit = await geocode.locate(place.name, near)
    except AppError:
        # The lookup being down is not a reason to refuse the link. The
        # place is saved with its name and no position, which is exactly
