...
export function PlanTrip() {
  ...
  if (!bundle.data || !tripId)
    return <main className="page">{t("common.error")}</main>;
      ...
      <main className="page stack">
        {plan === null ? (
          <>
            ...
            {done !== null && (
              <p className="hint">{count("trip_plan.applied", done)}</p>
            )}
          ...
          <>
            <p className="muted small">
              {count("trip_plan.summary", plan.writes.length, {
                days: plan.days.filter(
                  (day) => (day.plan?.visits.length ?? 0) > 0,
                ).length,
              })}
            </p>
            ...
            {plan.kept.length > 0 && (
              <p className="muted small">
                {count("trip_plan.kept", plan.kept.length)}
              </p>
            )}
              ...
              {plan.days.map((day) => (
                <div key={day.key} className="day">
                  <h3 className="day__header">
                    ...
                    {day.stop && (
                      <span className="day__count">{day.stop.name}</span>
                    )}
                  </h3>
                  {day.refused ? (
                    <p className="muted small">
                      {t(`trip_plan.day.${day.refused}`)}
                    </p>
                      ...
                      {day.plan?.visits.map((visit) => (
                        <li key={visit.id} className="plan__row">
                          ...
                          <span className="plan__name">
                            {names.get(visit.id)}
                            {visit.hoursUnknown && (
                              <span className="plan__flag">?</span>
                            )}
                          </span>
